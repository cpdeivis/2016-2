...
int encontra_ligacao(Grafo * grafo, Vertice * vertice_origem, Vertice * vertice_destino){
	int cont = 0;
	Vertice ** vertices_saidas = grafo_busca_vertices_saida(grafo, vertice_origem, &cont);
	if(vertices_saidas != NULL){
		int i, aux;
		for(i=aux=0;i<cont && !aux; i++){
			if(!(vertices_saidas[i] == vertice_destino))
				aux = encontra_ligacao(grafo, vertices_saidas[i], vertice_destino);
			else
				aux = 1;
		}
		return aux;
...
int main(){
	...
	//grafo_imprime_tab(grafo, vJoao);
	imprime_tab(grafo, vJoao);
	...
	else printf("%s e %s nao estao conectados\n", grafo_retorna_nome(vOrigem), grafo_retorna_nome(vDestino));
